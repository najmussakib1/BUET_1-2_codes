...
void print(arrayList* list) {
    ...
    if (list->size_value == 0) {
        ...
        for (int i = 0; i < list->size_value; i++) {
            if (i == list->curr_index) printf("| ");
            printf("%d ", list->array[i]);
        }
        if (list->curr_index == list->size_value) printf("| ");
    }
    printf("]\n");
}
...
void insert(int item, arrayList* list) {
    ...
    for (int i = list->size_value; i > list->curr_index; i--) {
        list->array[i] = list->array[i - 1];
    }
    list->array[list->curr_index] = item;
    list->size_value++;
    // In your sample, insert keeps the bar AFTER the new item
    list->curr_index++; 
}

int delete_cur(arrayList* list) {
    // Check if there is an item at the current cursor to delete
...
int size(arrayList* list) { return list->size_value; }
...
void next(int n, arrayList* list) {
    list->curr_index += n;
    if (list->curr_index > list->size_value) list->curr_index = list->size_value;
}
